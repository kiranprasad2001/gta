...
export function normalizeGoTransit(apiResponse: {
  ...
  return services.map((service): ArrivalPrediction => {
    ...
    const timeMinutes = Math.max(
      0,
      Math.round((departureTime.getTime() - now.getTime()) / 60000)
    );
    ...
    // Get platform info
    const platform =
      service.ActualPlatform || service.ScheduledPlatform || undefined;
  ...
  for (const pred of predictionsArray) {
    ...
    for (const dir of directions) {
      ...
      for (const eta of etas) {
        ...
        const lineMatch =
          dir.title?.match(/^(\d+\w*)/) || pred.routeTag?.match(/^(\d+\w*)/);
        ...
        predictions.push({
          line,
          destination,
          timeMinutes: Number.parseInt(eta.minutes, 10) || 0,
 ...
 */
export function normalizeTtcSubway(
  response: Array<{
    nextTrains: string;

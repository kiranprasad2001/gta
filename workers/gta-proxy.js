...
async function fetchGoTransit(stopCode, env) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: "Failed to fetch from Metrolinx",
...
/**
 * Fetch TTC Subway Next Train Arrival (NTAS) data
 */
async function fetchTtcSubway(stopCode) {
  const url = `https://ntas.ttc.ca/api/ntas/get-next-train-time/${encodeURIComponent(stopCode)}`;
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: "Failed to fetch from TTC NTAS",
        message: error.message,
      }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
...
export default {
  async fetch(request, env, _ctx) {
    ...
    switch (agency.toLowerCase()) {
      ...
      case "ttc-subway":
        response = await fetchTtcSubway(stopCode);
        break;
